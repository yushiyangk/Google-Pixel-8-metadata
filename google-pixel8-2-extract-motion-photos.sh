#!/usr/bin/env bash

# Extract standalone photo and video files from Google Pixel 8 motion photos
# Raw photos that are also a motion photo have the suffix ".RAW-01.MP.COVER.jpg"

# Requires:
# - jq
# - exiftool, https://exiftool.org/
# - motionminer from PyPI, patched to extract standalone photos, as in latest commit at https://github.com/yushiyangk/MotionMiner/tree/photo

INPUT_DIR="Photos-source/pixel8"
OUTPUT_VIDEO_DIR="Photos-source/pixel8_mp_video"
OUTPUT_PHOTO_DIR="Photos-source/pixel8_mp_photo"
LIVE_PHOTO_KEYWORD="Motion Photo"
LIVE_PHOTO_HIERARCHICAL_KEYWORD="__SOURCES|_phones|Google Pixel 8|stock camera|Motion Photo"

RAW_MOTION_PHOTO_JPG_SUFFIX=".RAW-01.MP.COVER.jpg"

motion_photos=()
counter=0

while read -r file; do
	if [ -n "$file" ]; then
		((counter++))

		input_file="$INPUT_DIR/$file"
		input_file_base_name="$(basename "$input_file" .jpg)"
		input_file_parent_dir="$(dirname "$input_file")"
		output_video_file="$OUTPUT_VIDEO_DIR/${file%.jpg}.video.mp4"
		output_photo_file="$OUTPUT_PHOTO_DIR/${file%.jpg}.photo.jpg"
		mkdir -p "$(dirname "$output_video_file")"
		mkdir -p "$(dirname "$output_photo_file")"

		if motionminer "$input_file" -o "$output_video_file" -p "$output_photo_file"; then
			# Write keyword to original image
			exiftool \
				-IPTC:Keywords-="$LIVE_PHOTO_KEYWORD" -IPTC:Keywords+="$LIVE_PHOTO_KEYWORD" \
				-XMP-dc:Subject-="$LIVE_PHOTO_KEYWORD" -XMP-dc:Subject+="$LIVE_PHOTO_KEYWORD" \
				-XMP-lr:WeightedFlatSubject-="$LIVE_PHOTO_KEYWORD" -XMP-lr:WeightedFlatSubject+="$LIVE_PHOTO_KEYWORD" \
				-XMP-lr:HierarchicalSubject-="$LIVE_PHOTO_HIERARCHICAL_KEYWORD" -XMP-lr:HierarchicalSubject+="$LIVE_PHOTO_HIERARCHICAL_KEYWORD" \
				-IPTCDigest=new -overwrite_original "$input_file"

			# Write keyword to any XMP sidecar files
			readarray -d '' xmp_files < <( find "$input_file_parent_dir" -type f -name "$input_file_base_name"'.[xX][mM][pP]' -print0 )
			for xmp_file in "${xmp_files[@]}"; do
				exiftool \
					-IPTC:Keywords-="$LIVE_PHOTO_KEYWORD" -IPTC:Keywords+="$LIVE_PHOTO_KEYWORD" \
					-XMP-dc:Subject-="$LIVE_PHOTO_KEYWORD" -XMP-dc:Subject+="$LIVE_PHOTO_KEYWORD" \
					-XMP-lr:WeightedFlatSubject-="$LIVE_PHOTO_KEYWORD" -XMP-lr:WeightedFlatSubject+="$LIVE_PHOTO_KEYWORD" \
					-XMP-lr:HierarchicalSubject-="$LIVE_PHOTO_HIERARCHICAL_KEYWORD" -XMP-lr:HierarchicalSubject+="$LIVE_PHOTO_HIERARCHICAL_KEYWORD" \
					-IPTCDigest=new -overwrite_original "$xmp_file"
			done

			# Write keyword to extracted photo and remove motion photo metadata
			#  Sample directory_json value:
			#   [
			#     {
			#       "Item": {
			#         "Mime": "image/jpeg",
			#         "Semantic": "Primary"
			#       }
			#     },
			#     {
			#       "Item": {
			#         "Length": 50519,
			#         "Mime": "image/jpeg",
			#         "Semantic": "GainMap"
			#       }
			#     }
			#   ]
			directory_json="$(exiftool -j -struct -XMP-GContainer:ContainerDirectory "$output_photo_file" | jq '.[0].ContainerDirectory | del(.[] | select(.Item.Semantic == "MotionPhoto"))')"
			directory_struct="$( echo "$directory_json" | jq 'map(.Item | to_entries | map(join("=")) | join(",") | "{Item={" + . + "}}") | join(",") | "[" + . + "]"' --raw-output )"
			exiftool \
				-IPTC:Keywords-="$LIVE_PHOTO_KEYWORD" -IPTC:Keywords+="$LIVE_PHOTO_KEYWORD" \
				-XMP-dc:Subject-="$LIVE_PHOTO_KEYWORD" -XMP-dc:Subject+="$LIVE_PHOTO_KEYWORD" \
				-XMP-lr:WeightedFlatSubject-="$LIVE_PHOTO_KEYWORD" -XMP-lr:WeightedFlatSubject+="$LIVE_PHOTO_KEYWORD" \
				-XMP-lr:HierarchicalSubject-="$LIVE_PHOTO_HIERARCHICAL_KEYWORD" -XMP-lr:HierarchicalSubject+="$LIVE_PHOTO_HIERARCHICAL_KEYWORD" \
				-XMP-GContainer:ContainerDirectory="$directory_struct" \
				-IPTCDigest=new -overwrite_original "$output_photo_file"

			motion_photos+=("$input_file")
		fi

	fi
done <<< "$(find "$INPUT_DIR" -type f -name '*'"$RAW_MOTION_PHOTO_JPG_SUFFIX" -printf '%P\n')"

find "$OUTPUT_VIDEO_DIR" -type d -empty -delete
find "$OUTPUT_PHOTO_DIR" -type d -empty -delete

echo ""
if [ ${#motion_photos[@]} -ne 0 ]; then
	echo "Found and extracted ${#motion_photos[@]} motion photos out of $counter original *.jpg files:"
	for photo in "${motion_photos[@]}"; do
		echo "$photo"
	done
	echo ""
	echo "Extracted to $OUTPUT_VIDEO_DIR and $OUTPUT_PHOTO_DIR"
else
	echo "Found and extracted 0 motion photos out of $counter original *.jpg files"
fi
